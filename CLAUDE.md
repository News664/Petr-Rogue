...
3. Merge to `main` via MCP — create a PR then merge it:
   ...
   mcp__github__merge_pull_request({ pullNumber: N, merge_method: 'rebase' })
   ```
...
**Never** try to fix the proxy or force-push to main. Never use MCP push_files as a workaround for feature branches — direct git push works there.
...
assets/
  {charId}/           — avatar.png, sprite.png, Portrait_0/25/50/75/100.png
