...
Character art for `galatea`. **Status: needs art** — this folder is empty; every
file below must be generated. Galatea is a newly added character. Female (lore rule).

Place all files in `assets/galatea/`.
...
Galatea also needs card art (`assets/cards/galatea/`) and game-over art
(`assets/game-over/{cause-key}-galatea.png`) — see those folders' READMEs.
