...
- Resource: **Geodes** (`player.geodes`) — crystallize Petrify into Geodes, then
  ...
  2–3 cost consumers (shatter_burst, mother_lode, cataclysm) dump it for burst.
- Unique cards: ore_strike, crystallize, facet_strike, geode_ward, splinter,
  shatter_burst, prismatic_core, mother_lode, grand_geode, cataclysm

### Galatea — "The Statue" (75 HP, 3 Energy)
- Starting relic: **Sculptor's Plinth** — +3 Poise at end of any turn with no Attack played
- Resource: **Poise** (`player.poise`) — built by restraint/defense, spent by
  attacks for scaling. **Harden** keyword: a `retained` card whose value rises
  each turn it's held (onRetain) and resets when played.
- Unique cards: composure, brace, chisel, unyielding, hold_fast, monument,
  pedestal, weight_of_ages, living_marble, awakening

Geodes/Poise are combat-scoped (reset in `startCombat`), shown as HUD badges.
...
### Cross-combat status carry (`pendingStatuses`)
`startCombat` wipes `player.statusEffects`, so statuses applied *out of combat*
(events) would vanish. Events instead set `player.pendingStatuses` (e.g. Numbing);
`startCombat` applies these after the wipe, then clears them.

### Asset generation (Codex collaboration)
Art is generated by Codex on a separate branch. Codex writes only `assets/**`
(see `assets/ART_TODO.md` checklist + `assets/README.md` art direction); Claude
owns `src/**` + `sw.js` and regenerates the offline precache after art lands.
